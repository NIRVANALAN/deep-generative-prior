...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.log_size = int(math.log(size, 2))  # 7 if size=128
    ...
    def get_params(self, index=0, update_embed=False):
        if index == 0:
            ...
            for param in self.convs[2 * (index - 2)].parameters():
                yield param
            for param in self.convs[2 * (index - 2) + 1].parameters():
                yield param
...
class Discriminator(nn.Module):
    ...
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], *args, **kwargs):
        super().__init__()

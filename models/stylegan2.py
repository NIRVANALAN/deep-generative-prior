...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.style_conv1 = StyleConv(
            ...
            resample_kernel=resample_kernel)
        self.to_rgb1 = ToRGB(
            channels['4'], num_style_feat, upsample=False, resample_kernel=resample_kernel)
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.style_convs.append(
                StyleConv(
                    ...
                    resample_kernel=resample_kernel))
            self.to_rgbs.append(
                ToRGB(out_channels, num_style_feat, upsample=True, resample_kernel=resample_kernel))
    ...
    def get_params(self, index=0, update_embed=False):
        if index == 0:
            for param in self.style_mlp.parameters():
                yield param
        elif index == 1:
            for param in self.style_conv1.parameters():
                yield param
            for param in self.to_rgb1.parameters():
                yield param
        else:
            for param in self.style_convs[2 * (index - 2)].parameters():
                yield param
            for param in self.style_convs[2 * (index - 2) + 1].parameters():
                yield param
            for param in self.to_rgbs[index - 2].parameters():
                yield param
    ...
    def forward(self,
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(self.style_convs[::2],
                                                        self.style_convs[1::2], noise[1::2],
...
class Discriminator(nn.Module):
    ...
    def __init__(self,
        ...
        conv_body = [ConvLayer(3, channels[f'{size}'], 1, bias=True, activate=True)]
        ...
        self.conv_body = nn.ModuleList(conv_body)
    ...
    def forward(self, x):
        ...
        group = min(
            b, self.stddev_group)  # Minibatch must be divisible by (or smaller than) group_size
